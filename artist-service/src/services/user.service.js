...
const getArtistById = async (artistId) => {
    try {
        ...
        if (err.message === 'Artist not found' || err.message === 'Invalid artist ID') {
            throw err;
        }
...
const updateArtist = async (artistId, updateData, req) => {
    try {
        ...
        if (updateData.first_name) {
            ...
            if (sanitizedData.first_name.length < 1 || sanitizedData.first_name.length > 100) {
        ...
        if (updateData.last_name) {
            ...
            if (sanitizedData.last_name.length < 1 || sanitizedData.last_name.length > 100) {
        ...
        if (err.message.includes('Artist not found') ||
...
const changePassword = async (artistId, currentPassword, newPassword) => {
    try {
        ...
        const artist = await userRepository.getUserForPasswordVerification(artistId);
        if (!artist) {
            throw new Error('Artist not found');
        }

        // Verify current password
        const isValidPassword = await bcrypt.compare(currentPassword, artist.password);
        if (!isValidPassword) {
...
const uploadAvatar = async (artistId, file) => {
    try {
        ...
        const avatarUrl = `/uploads/avatars/${artistId}_${Date.now()}_${file.originalname}`;
